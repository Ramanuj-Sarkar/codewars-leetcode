# You have a matrix with values and you can travel to adjacent squares.
# You have to find the longest strictly increasing path (not equal).
class Solution:
    def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
        m, n = len(matrix), len(matrix[0])
        
        # create a dp array of size m * n
        # to store already computed max_increasing_path_length for index (i, j)
